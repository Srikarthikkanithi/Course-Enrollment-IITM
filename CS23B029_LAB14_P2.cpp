...
class Coursereg{
      ...
      unordered_map<string,queue<string>> m4;
      public:
    //   Coursereg(){
    //     m1.clear();
    //     m2.clear();
    ...
    //   }
      ...
      void drop_course(string& s1,string& s2){
           auto itr1=m1.find(s1);
           auto itr2=m2.find(s2);
           if(itr1!=m1.end()&&itr2!=m2.end()){
             auto itr3=m3.find(s1);
             if(itr3!=m3.end()){
             vector<string>& v=itr3->second;
             auto itr4=find(v.begin(),v.end(),s2);
             if(itr4!=v.end()){
                //cout<<*itr4<<endl;
             itr3->second.erase(itr4);
             itr1->second.capacity++;
             auto itr5=m4.find(s1);
             if(itr5!=m4.end()){
             while(!(itr5->second.empty())&&itr1->second.capacity!=0){
                   enroll(s1,itr5->second.front()); 
                   itr5->second.pop();
             }
             }
           }
